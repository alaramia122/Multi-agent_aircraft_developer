...
class SqlAlchemyWorkspaceRegistry(_TransactionAware):
    ...
    async def update(self, workspace: Workspace) -> Workspace:
        ...
        resetting_validation = record.validation_graph_hash is not None and workspace.validation_graph_hash is None and not workspace.validation_evidence and workspace.state is WorkspaceState.ACTIVE
        if record.validation_graph_hash is not None and record.validation_graph_hash != workspace.validation_graph_hash and not resetting_validation:
        ...
        if record.validation_evidence != workspace.validation_evidence and not resetting_validation:
            raise ValueError("validation evidence is immutable once bound outside active reset")
        if record.reconciled and not workspace.reconciled and current_state is not WorkspaceState.ACTIVE:
