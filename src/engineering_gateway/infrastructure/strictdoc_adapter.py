...
class LocalStrictDocAdapter:
    ...
    The adapter deliberately delegates SDoc parsing to StrictDoc itself instead of
    implementing a second SDoc parser in the Gateway. StrictDoc's documented JSON
    export is the integration boundary; the Gateway only maps the exported
    authoritative requirement identity into its canonical reference model.
    """
    ...
    def _export_documents(self) -> list[dict[str, Any]]:
        output_dir = self._project_path / ".engineering_gateway_strictdoc_export"
