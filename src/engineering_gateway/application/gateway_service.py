...
class GatewayApplicationService:
    ...
    async def validate(self, actor: Actor, elements: list[EngineeringElement], relations: list[EngineeringRelation], profile_id: str, profile_version: str) -> ValidationResult:
        self._require_read(actor)
        ...
        await self._record(actor, action="validate", target_type="standard_profile", result=AuditResult.SUCCESS, reason="validation passed" if result.valid else f"validation found {len(result.issues)} issue(s)", metadata={"issue_count": len(result.issues), "graph_hash": result.graph_hash})
    ...
    async def create_workspace(self, actor: Actor, baseline_id: UUID, change_request_id: UUID, git_ref: str = "HEAD") -> Workspace:
        ...
        workspace = Workspace(source_baseline_id=baseline.id, source_git_commit=baseline.git_commit, change_request_id=change_request.id, git_ref=git_ref)
        try:
            ...
            change_request = change_request.model_copy(update={"state": ChangeRequestState.IN_PROGRESS, "source_baseline_id": baseline.id, "workspace_id": workspace.id})
            await self._change_requests.update(change_request)
            created = await self._workspaces.create(workspace)
        except ValueError as exc:
            await self._record(actor, action="create_workspace", target_type="workspace", target_id=workspace.id, result=AuditResult.FAILURE, reason=str(exc))
            raise GatewayServiceError(str(exc)) from exc
        await self._record(actor, action="create_workspace", target_type="workspace", target_id=created.id, result=AuditResult.SUCCESS, metadata={"source_baseline_id": str(baseline.id), "source_git_commit": baseline.git_commit, "change_request_id": str(change_request.id), "git_ref": git_ref})
        return created
    ...
    async def reopen_workspace(self, actor: Actor, workspace_id: UUID) -> None:
        ...
        await self._record(actor, action="reopen_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.SUCCESS)
    ...
    async def close_workspace(self, actor: Actor, workspace_id: UUID) -> None:
        ...
        await self._record(actor, action="close_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.SUCCESS)
    ...
    async def save_workspace_element(self, actor: Actor, element: EngineeringElement, workspace_id: UUID) -> EngineeringElement:
        ...
        await self._record(actor, action="save_workspace_element", target_type="engineering_element", target_id=element.id, result=AuditResult.SUCCESS, metadata={"workspace_id": str(workspace_id)})
        return saved
    ...
    async def add_workspace_relation(self, actor: Actor, relation: EngineeringRelation, workspace_id: UUID) -> EngineeringRelation:
        ...
        await self._record(actor, action="add_workspace_relation", target_type="engineering_relation", target_id=relation.id, result=AuditResult.SUCCESS, metadata={"workspace_id": str(workspace_id)})
        return saved
    ...
    async def approve(self, actor: Actor, baseline_id: UUID) -> None:
        try:
            ...
            await self._record(actor, action="approve_baseline", target_type="baseline", target_id=baseline_id, result=AuditResult.DENIED, reason=str(exc))
            raise GatewayServiceError(str(exc)) from exc
    ...
    async def approve_workspace(self, actor: Actor, workspace_id: UUID) -> Baseline:
        ...
        try:
            ...
            await self._record(actor, action="approve_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.DENIED, reason=str(exc))
            raise GatewayServiceError(str(exc)) from exc
        workspace, change_request = await self._load_workflow(workspace_id)
        if workspace.state is not WorkspaceState.READY_FOR_APPROVAL or change_request.state is not ChangeRequestState.READY_FOR_APPROVAL:
        ...
        try:
            ...
            baseline = Baseline(name=tag, git_repository=tagged.repository, git_commit=tagged.commit, git_tag=tagged.tag or tag, external_versions=tuple(ExternalSystemVersion(system=v.system, version=v.version) for v in workspace.reconciliation_external_versions))
            ...
            await self._record(actor, action="approve_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.FAILURE, reason=str(exc))
    ...
    async def _get_active_profile(self, profile_id: str, profile_version: str, *, actor: Actor, action: str):
        ...
        if not await self._profiles.is_active(profile_id, profile_version):
            await self._record(actor, action=action, target_type="standard_profile", result=AuditResult.FAILURE, reason=f"profile '{profile_id}@{profile_version}' is not active")
