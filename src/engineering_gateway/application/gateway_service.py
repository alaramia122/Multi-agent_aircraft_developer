...
class GatewayApplicationService:
    ...
    async def get_element(self, actor: Actor, element_id: UUID) -> EngineeringElement | None:
        ...
        await self._record(
            actor, "get_element", "engineering_element", element_id, AuditResult.SUCCESS
        )
        return element
    ...
    async def get_relations(self, actor: Actor, element_id: UUID) -> list[EngineeringRelation]:
        ...
        await self._record(
            actor, "get_relations", "engineering_element", element_id, AuditResult.SUCCESS
        )
        return relations
    ...
    async def validate(
        ...
        await self._record(
            ...
            if result.valid
            else f"validation found {len(result.issues)} issue(s)",
    ...
    async def create_workspace(
        ...
        if self._baselines is None or self._change_requests is None or self._workspaces is None:
            raise GatewayServiceError(
                "baseline/change-request/workspace registries are not configured"
            )
        ...
        if change_request.source_baseline_id not in (None, baseline.id):
            raise GatewayServiceError(
                "change request source baseline does not match workspace baseline"
            )
    ...
    async def reject_workspace(self, actor: Actor, workspace_id: UUID, reason: str) -> None:
        ...
        await self._record(
            actor, "reject_workspace", "workspace", workspace_id, AuditResult.SUCCESS, reason=reason
        )
    ...
    async def reopen_workspace(self, actor: Actor, workspace_id: UUID) -> None:
        ...
        if (
            ...
            or change_request.state is not ChangeRequestState.REJECTED
        ):
            raise GatewayServiceError(
                "only a rejected change request with an active workspace can be reopened"
            )
        ChangeGate.require_transition(change_request.state, ChangeRequestState.IN_PROGRESS)
        await self._change_requests.update(
        ...
        await self._record(
            actor, "reopen_workspace", "workspace", workspace_id, AuditResult.SUCCESS
        )
    ...
    async def close_workspace(self, actor: Actor, workspace_id: UUID) -> None:
        ...
        if workspace.state is WorkspaceState.APPROVED:
            if change_request.state is not ChangeRequestState.APPROVED:
                raise GatewayServiceError(
                    "approved workspace requires an approved change request before closure"
                )
            ChangeGate.require_transition(change_request.state, ChangeRequestState.CLOSED)
        elif (
            workspace.state is WorkspaceState.ACTIVE
            and change_request.state is ChangeRequestState.REJECTED
        ):
            ChangeGate.require_transition(change_request.state, ChangeRequestState.CLOSED)
        else:
            raise GatewayServiceError("workspace can be closed only after approval or rejection")
        await self._change_requests.update(
            change_request.model_copy(update={"state": ChangeRequestState.CLOSED})
        )
    ...
    async def approve(self, actor: Actor, baseline_id: UUID) -> None:
        try:
            ...
            raise GatewayServiceError(str(exc)) from exc
        raise GatewayServiceError(
            "direct baseline approval is disabled; approve a ready workspace instead"
        )
    ...
    async def approve_workspace(self, actor: Actor, workspace_id: UUID) -> Baseline:
        if (
            ...
            or self._baselines is None
            or self._git is None
        ):
            raise GatewayServiceError(
                "baseline/change-request/workspace/Git services are not configured"
            )
        try:
            ChangeGate.require_approval(actor.authorization_level, actor_is_ai=actor.is_ai)
        except ValueError as exc:
        ...
        if (
            ...
            raise GatewayServiceError(
                "workspace and change request must both be ready for approval"
            )
        ...
        try:
            ...
            baseline = Baseline(
                ...
                external_versions=tuple(
                    ...
                    for v in workspace.reconciliation_external_versions
                ),
            )
            ...
            await self._workspaces.update(
                workspace.model_copy(update={"state": WorkspaceState.APPROVED})
            )
            await self._change_requests.update(
        ...
        await self._record(
            ...
            metadata={
                ...
                "external_versions": [
                    {"system": v.system, "version": v.version} for v in registered.external_versions
                ],
            },
        )
        return registered
    ...
    async def _get_active_profile(
        self, profile_id: str, profile_version: str, *, actor: Actor, action: str
    ) -> StandardProfile:
