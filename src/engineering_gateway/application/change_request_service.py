...
from engineering_gateway.domain.ports import AuditSink, ChangeRequestRegistryPort
    ...
    async def create_change_request(
        ...
        if actor.is_ai or actor.authorization_level != AuthorizationLevel.L2_MODIFY_WORKSPACE:
            raise GatewayServiceError(
                "change-request creation requires human L2 workspace modification authority"
            )
        try:
