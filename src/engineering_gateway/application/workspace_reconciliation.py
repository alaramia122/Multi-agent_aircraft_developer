...
class WorkspaceReconciliationService:
    ...
    async def reconcile(self, actor: Actor, workspace_id: UUID) -> ReconciliationResult:
        ...
        if workspace.state is not WorkspaceState.READY_FOR_APPROVAL or change_request.state is not ChangeRequestState.READY_FOR_APPROVAL:
            raise GatewayServiceError("workspace and change request must be ready for approval before reconciliation")
        ...
        try:
            ...
            await self._audit.record(AuditEvent(actor_id=actor.actor_id, actor_type=actor.actor_type, authorization_level=actor.authorization_level, action="reconcile_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.FAILURE, reason=str(exc), metadata={"change_elements": len(changes.elements), "change_relations": len(changes.relations), "change_set_hash": change_set_hash}))
            if isinstance(exc, GatewayServiceError):
                raise
            raise GatewayServiceError(f"workspace reconciliation failed: {exc}") from exc

        await self._audit.record(AuditEvent(actor_id=actor.actor_id, actor_type=actor.actor_type, authorization_level=actor.authorization_level, action="reconcile_workspace", target_type="workspace", target_id=workspace_id, result=AuditResult.SUCCESS, metadata={"change_elements": len(changes.elements), "change_relations": len(changes.relations), "change_set_hash": change_set_hash, "external_versions": [{"system": version.system, "version": version.version} for version in versions]}))
        return ReconciliationResult(workspace_id=workspace_id, change_set_hash=change_set_hash, external_versions=versions)
