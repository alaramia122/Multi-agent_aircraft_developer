...
class StandardProfileApplicationService:
    ...
    async def register(self, actor: Actor, profile: StandardProfile) -> None:
        if actor.is_ai or actor.authorization_level != AuthorizationLevel.L3_APPROVE:
            raise GatewayServiceError(
                "Standard Profile registration requires a human L3 governance actor"
            )
        try:
    ...
    async def activate(self, actor: Actor, profile_id: str, version: str) -> StandardProfile:
        if actor.is_ai or actor.authorization_level != AuthorizationLevel.L3_APPROVE:
            raise GatewayServiceError(
                "Standard Profile activation requires a human L3 governance actor"
            )
        try:
            profile = await self._registry.activate(profile_id, version)
        except ValueError as exc:
            await self._record_identity(
                actor, "activate_profile", profile_id, version, AuditResult.FAILURE, str(exc)
            )
            raise GatewayServiceError(str(exc)) from exc
        await self._record_identity(
            actor, "activate_profile", profile_id, version, AuditResult.SUCCESS
        )
        return profile
    ...
    async def deactivate(self, actor: Actor, profile_id: str, version: str) -> None:
        if actor.is_ai or actor.authorization_level != AuthorizationLevel.L3_APPROVE:
            raise GatewayServiceError(
                "Standard Profile deactivation requires a human L3 governance actor"
            )
        try:
            await self._registry.deactivate(profile_id, version)
        except ValueError as exc:
            await self._record_identity(
                actor, "deactivate_profile", profile_id, version, AuditResult.FAILURE, str(exc)
            )
            raise GatewayServiceError(str(exc)) from exc
        await self._record_identity(
            actor, "deactivate_profile", profile_id, version, AuditResult.SUCCESS
        )

    async def compose(
        self,
        actor: Actor,
        profiles: list[StandardProfile],
        *,
        profile_id: str,
        version: str,
        name: str,
        ...
        if actor.is_ai or actor.authorization_level != AuthorizationLevel.L3_APPROVE:
            raise GatewayServiceError(
                "Standard Profile composition requires a human L3 governance actor"
            )
        try:
            composed = StandardProfileEngine.compose(
                profiles, id=profile_id, version=version, name=name
            )
            await self._registry.register(composed)
        except (ValueError, ProfileCompositionError) as exc:
            await self._record_identity(
                actor, "compose_profile", profile_id, version, AuditResult.FAILURE, str(exc)
            )
            raise GatewayServiceError(str(exc)) from exc
        await self._record_identity(
            actor, "compose_profile", profile_id, version, AuditResult.SUCCESS
        )
        return composed
