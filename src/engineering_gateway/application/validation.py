...
class DeterministicValidationEngine:
    ...
    def validate_graph(
        ...
        *,
        ...
        for gap in traceability.traceability_violations(profile):
            issues.append(
                ValidationIssue(
                    ValidationIssueCode.FORBIDDEN_TRACEABILITY
                    if gap.forbidden
                    else ValidationIssueCode.MISSING_TRACEABILITY,
        ...
        self._validate_lifecycle(
            profile, graph, element_map, lifecycle_states, lifecycle_transitions, issues
        )
        ...
        for element in graph.elements:
            ...
            for attribute_id in sorted(values):
                if attribute_id not in definitions:
                    issues.append(
                        ValidationIssue(
                            ValidationIssueCode.UNKNOWN_ATTRIBUTE,
                            f"Attribute '{attribute_id}' is not defined for element type '{element.type_id}'",
                            element_id=element.id,
                        )
                    )
            for attribute in definition.attributes:
                if attribute.required and attribute.id not in values:
                    issues.append(
                        ValidationIssue(
                            ValidationIssueCode.MISSING_REQUIRED_ATTRIBUTE,
                            f"Required attribute '{attribute.id}' is missing for element '{element.id}'",
                            element_id=element.id,
                        )
                    )
                elif attribute.id in values and not _attribute_value_matches(
                    attribute.type, values[attribute.id]
                ):
                    issues.append(
                        ValidationIssue(
                            ValidationIssueCode.INVALID_ATTRIBUTE_TYPE,
                            f"Attribute '{attribute.id}' has an invalid value type for '{attribute.type}'",
                            element_id=element.id,
                        )
                    )
        for element_id in sorted(
            set(attributes) - {element.id for element in graph.elements}, key=str
        ):
            issues.append(
                ValidationIssue(
                    ValidationIssueCode.UNKNOWN_ATTRIBUTE,
                    f"Attributes are supplied for unknown element '{element_id}'",
    ...
    def _validate_lifecycle(
        ...
        lifecycle_transitions: dict[UUID, tuple[str, str]] | None,
        issues: list[ValidationIssue],
    ) -> None:
                    ...
                    if element.type_id == lifecycle.element_type_id:
                        issues.append(
                            ValidationIssue(
                                ...
                                f"Lifecycle '{lifecycle.id}' requires explicit state for element '{element.id}'",
                                element_id=element.id,
        ...
        if lifecycle_states is not None:
            for lifecycle in profile.lifecycles:
                for element in graph.elements:
                    if (
                        element.type_id == lifecycle.element_type_id
                        and element.id not in lifecycle_states
                    ):
                        issues.append(
                            ValidationIssue(
                                ValidationIssueCode.INVALID_LIFECYCLE_STATE,
                        ...
                        element.type_id == lifecycle.element_type_id
                        and element.id in lifecycle_states
                        and lifecycle_states[element.id] not in lifecycle.states
                    ):
                        issues.append(
                            ValidationIssue(
                                ValidationIssueCode.INVALID_LIFECYCLE_STATE,
                                f"Lifecycle '{lifecycle.id}' does not define state '{lifecycle_states[element.id]}'",
                                element_id=element.id,
                                rule_id=lifecycle.id,
                            )
                        )
            ...
            for lifecycle in profile.lifecycles:
                for element in graph.elements:
                    ...
                    if source not in lifecycle.states or target not in lifecycle.states:
                        issues.append(
                            ValidationIssue(
                                ValidationIssueCode.INVALID_LIFECYCLE_TRANSITION,
                                f"Lifecycle '{lifecycle.id}' transition '{source}' -> '{target}' references an unknown state",
                                element_id=element.id,
                                rule_id=lifecycle.id,
                            )
                        )
                        ...
                        issues.append(
                            ValidationIssue(
                                ValidationIssueCode.INVALID_LIFECYCLE_TRANSITION,
                                f"Lifecycle '{lifecycle.id}' does not allow transition '{source}' -> '{target}'",
                                element_id=element.id,
                                rule_id=lifecycle.id,
                            )
                        )
        ...
        for element_id in sorted(evidence_ids - set(element_map), key=str):
            issues.append(
                ValidationIssue(
                    ValidationIssueCode.INVALID_LIFECYCLE_STATE,
                    f"Lifecycle evidence is supplied for unknown element '{element_id}'",
                    element_id=element_id,
                )
            )
    ...
    def _validate_artifacts(
        ...
        for requirement in profile.artifacts:
            ...
            for element in graph.elements:
                if (
                    element.type_id == requirement.element_type_id
                    and (element.id, requirement.artifact_type) not in artifact_evidence
                ):
                    issues.append(
                        ValidationIssue(
    ...
    *,
    ...
    payload = {
        ...
        "elements": [
            ...
            for element in sorted(graph.elements, key=lambda item: str(item.id))
        ],
        "relations": [
        ...
        "attributes": sorted(
            [
                [str(element_id), key, value]
                for element_id, values in (attributes or {}).items()
                for key, value in values.items()
            ],
            key=lambda item: (
                item[0],
                item[1],
                json.dumps(item[2], ensure_ascii=False, sort_keys=True, default=str),
            ),
        ),
...
def _issue_sort_key(issue: ValidationIssue) -> tuple[str, str, str, str]:
    return (
        issue.code,
        str(issue.element_id or ""),
        str(issue.relation_id or ""),
        issue.rule_id or "",
    )
