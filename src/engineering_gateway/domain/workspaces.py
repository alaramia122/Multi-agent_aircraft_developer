...
class Workspace(BaseModel):
    ...
    def bind_profile(self, profile_id: str, profile_version: str) -> "Workspace":
        ...
        return self.model_copy(
            update={"profile_id": profile_id, "profile_version": profile_version}
        )
    ...
    def bind_validation_evidence(self, graph_hash: str, evidence: dict[str, object]) -> "Workspace":
        ...
        return self.model_copy(
            update={"validation_graph_hash": graph_hash, "validation_evidence": evidence}
        )
    ...
    def mark_reconciled(
        self, change_set_hash: str, external_versions: tuple[ExternalVersion, ...] = ()
    ) -> "Workspace":
        ...
        return self.model_copy(
            update={
                "reconciled": True,
                "reconciled_change_set_hash": change_set_hash,
                "reconciliation_external_versions": external_versions,
            }
        )
    ...
    def require_reconciled(self, current_change_set_hash: str | None = None) -> None:
        ...
        if (
            current_change_set_hash is not None
            and self.reconciled_change_set_hash != current_change_set_hash
        ):
            raise ValueError(
                "workspace reconciliation evidence is stale for the current change-set"
            )
...
class WorkspaceGate:
    ...
    _TRANSITIONS: dict[WorkspaceState, frozenset[WorkspaceState]] = {
        WorkspaceState.ACTIVE: frozenset(
            {WorkspaceState.READY_FOR_APPROVAL, WorkspaceState.CLOSED}
        ),
        WorkspaceState.READY_FOR_APPROVAL: frozenset(
            {WorkspaceState.ACTIVE, WorkspaceState.APPROVED}
        ),
...
class WorkspaceRegistry:
    ...
    async def update(self, workspace: Workspace) -> Workspace:
        ...
        if current.profile_id is not None and current.profile_id != workspace.profile_id:
            raise ValueError("workspace validation profile is immutable once bound")
        if (
            current.profile_version is not None
            and current.profile_version != workspace.profile_version
        ):
            raise ValueError("workspace validation profile is immutable once bound")
        resetting_validation = (
            current.validation_graph_hash is not None
        ...
        if (
            current.validation_graph_hash is not None
            and current.validation_graph_hash != workspace.validation_graph_hash
            and not resetting_validation
        ):
            raise ValueError("validation evidence is immutable once bound outside active reset")
        if (
            current.validation_graph_hash is not None
            and current.validation_evidence != workspace.validation_evidence
            and not resetting_validation
        ):
            raise ValueError("validation evidence is immutable once bound outside active reset")
        if (
        ...
        if (
            ...
            and (
                current.reconciled_change_set_hash != workspace.reconciled_change_set_hash
                or current.reconciliation_external_versions
                != workspace.reconciliation_external_versions
            )
        ):
            raise ValueError(
