...
def test_registry_round_trip_and_deterministic_listing() -> None:
    ...
    assert registry.list() == sorted([first, second], key=lambda item: str(item.id))


def test_registry_rejects_different_duplicate_identity() -> None:
    registry = BaselineRegistry()
    baseline = make_baseline()
...
def test_registry_is_idempotent_for_same_baseline() -> None:
    registry = BaselineRegistry()
    baseline = make_baseline()
