...
def make_element(type_id: str, kind: ElementKind) -> EngineeringElement:
    return EngineeringElement(
        kind=kind, type_id=type_id, name=type_id, external_system="test", external_id=str(uuid4())
    )
...
def test_unknown_element_type_is_reported() -> None:
    result = DeterministicValidationEngine().validate(
        [make_element("unknown", ElementKind.REQUIREMENT)], [], make_profile()
    )
...
def test_element_kind_mismatch_is_reported() -> None:
    result = DeterministicValidationEngine().validate(
        [make_element("architecture", ElementKind.REQUIREMENT)], [], make_profile()
    )
...
def test_dangling_relation_is_reported() -> None:
    ...
    relation = EngineeringRelation(
        source_id=source.id, relation_type=RelationType.SATISFIES, target_id=uuid4()
    )
...
def test_forbidden_relation_is_reported() -> None:
    source, target = (
        make_element("requirement", ElementKind.REQUIREMENT),
        make_element("architecture", ElementKind.ARCHITECTURE),
    )
    relation = EngineeringRelation(
        source_id=source.id, relation_type=RelationType.DERIVES_FROM, target_id=target.id
    )
...
def test_missing_required_traceability_is_reported() -> None:
    result = DeterministicValidationEngine().validate(
        [make_element("requirement", ElementKind.REQUIREMENT)], [], make_profile()
    )
...
def test_valid_graph_has_no_issues() -> None:
    ...
    relation = EngineeringRelation(
        source_id=source.id, relation_type=RelationType.SATISFIES, target_id=target.id
    )
    result = DeterministicValidationEngine().validate([source, target], [relation], make_profile())
...
def test_duplicate_relation_is_reported() -> None:
    ...
    result = DeterministicValidationEngine().validate(
        [source, target], [relation, relation.model_copy(update={"id": uuid4()})], make_profile()
    )
...
def test_external_identity_collision_is_reported() -> None:
    ...
    assert any(
        issue.code == ValidationIssueCode.DUPLICATE_EXTERNAL_IDENTITY for issue in result.issues
    )
...
def test_verification_relation_satisfies_rule() -> None:
    ...
    source, architecture, verification = (
        make_element("requirement", ElementKind.REQUIREMENT),
        make_element("architecture", ElementKind.ARCHITECTURE),
        make_element("verification", ElementKind.VERIFICATION),
    )
    relations = [
        EngineeringRelation(
            source_id=source.id, relation_type=RelationType.SATISFIES, target_id=architecture.id
        ),
        EngineeringRelation(
            source_id=source.id, relation_type=RelationType.VERIFIED_BY, target_id=verification.id
        ),
    ]
    result = DeterministicValidationEngine().validate(
        [source, architecture, verification], relations, profile
    )
    assert not any(
        issue.code == ValidationIssueCode.MISSING_VERIFICATION for issue in result.issues
    )
    ...
    return StandardProfile(
        ...
        element_types=[
            ElementTypeDefinition(
                ...
                attributes=[
                    AttributeDefinition(id="priority", type=AttributeType.INTEGER, required=True),
                    AttributeDefinition(id="safety", type=AttributeType.BOOLEAN),
                ],
            )
        ],
    )
...
def test_required_and_typed_attributes_are_validated() -> None:
    ...
    assert any(
        issue.code == ValidationIssueCode.MISSING_REQUIRED_ATTRIBUTE for issue in missing.issues
    )
    valid = DeterministicValidationEngine().validate(
        [element], [], profile, attributes={element.id: {"priority": 1, "safety": True}}
    )
...
def test_invalid_and_unknown_attributes_are_reported() -> None:
    ...
    result = DeterministicValidationEngine().validate(
        ...
        [],
        attribute_profile(),
        attributes={element.id: {"priority": True, "other": "x"}},
    )
...
def test_lifecycle_state_and_transition_are_validated() -> None:
    ...
    bad_state = DeterministicValidationEngine().validate(
        [element], [], profile, lifecycle_states={element.id: "unknown"}
    )
    assert any(
        issue.code == ValidationIssueCode.INVALID_LIFECYCLE_STATE for issue in bad_state.issues
    )
    bad_transition = DeterministicValidationEngine().validate(
        [element],
        [],
        profile,
        lifecycle_states={element.id: "approved"},
        lifecycle_transitions={element.id: ("approved", "draft")},
    )
    assert any(
        issue.code == ValidationIssueCode.INVALID_LIFECYCLE_TRANSITION
        for issue in bad_transition.issues
    )
    good = DeterministicValidationEngine().validate(
        [element],
        [],
        profile,
        lifecycle_states={element.id: "approved"},
        lifecycle_transitions={element.id: ("draft", "approved")},
    )
...
def test_required_artifact_is_validated_from_explicit_evidence() -> None:
    element = make_element("requirement", ElementKind.REQUIREMENT)
    profile = attribute_profile().model_copy(
        update={
            "artifacts": [
                ArtifactRequirement(
                    id="req-spec", element_type_id="requirement", artifact_type="specification"
                )
            ]
        }
    )
    missing = DeterministicValidationEngine().validate(
        [element], [], profile, attributes={element.id: {"priority": 1}}
    )
    ...
    valid = DeterministicValidationEngine().validate(
        ...
        [],
        profile,
        attributes={element.id: {"priority": 1}},
        artifact_evidence={(element.id, "specification")},
    )
    assert not valid.issues
...
def test_validation_hash_changes_when_evidence_changes() -> None:
    element = make_element("requirement", ElementKind.REQUIREMENT)
    profile = attribute_profile()
    first = DeterministicValidationEngine().validate(
        [element], [], profile, attributes={element.id: {"priority": 1}}
    )
    second = DeterministicValidationEngine().validate(
        [element], [], profile, attributes={element.id: {"priority": 2}}
    )
...
def test_graph_hash_is_stable_under_input_order() -> None:
    ...
    assert graph_hash(
        EngineeringGraph(elements=[source, target], relations=[relation]), profile
    ) == graph_hash(EngineeringGraph(elements=[target, source], relations=[relation]), profile)
